...
const getFrontMatter = (properties) => {
  ...
  // Convert url to Eleventy permalink before discarding
  if (properties.url) {
    try {
      const urlPath = new URL(properties.url).pathname;
      properties.permalink = urlPath.endsWith("/") ? urlPath : urlPath + "/";
    } catch {
      // url may be a relative path rather than a full URL
      const path = properties.url.startsWith("/")
        ? properties.url
        : "/" + properties.url;
      properties.permalink = path.endsWith("/") ? path : path + "/";
    }
  }
