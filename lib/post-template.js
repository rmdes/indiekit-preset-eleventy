...
const getFrontMatter = (properties) => {
  ...
  // properties.url may be absolute (https://example.com/path) or relative (/path)
  // Extract pathname to ensure only the path is used as permalink
  if (properties.url) {
    let urlPath;
    try {
      urlPath = new URL(properties.url).pathname;
    } catch {
      // Relative path — ensure it starts with /
      urlPath = properties.url.startsWith("/")
        ? properties.url
        : "/" + properties.url;
    }
